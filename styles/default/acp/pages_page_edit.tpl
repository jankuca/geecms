	<div id="content">
		<h2>{L_MODULE_PAGES} &mdash; {L_PAGES_PAGE_EDIT}: <strong>{PAGE.HEADER}</strong></h2>
		...
		<div>
			...
			<fieldset>
				...
				<input type="hidden" name="page[path_original]" value="{PAGE.PATH}" />
				
				<input type="hidden" name="page_slug_original" value="{POST.SLUG}" />
				<input type="radio" name="page_slug_generate" value="2" id="post_slug_keep_original" checked="checked" /> <label class="none" for="post_slug_keep_original">{L_SLUG_KEEP_ORIGINAL}</label><br />
				<input type="radio" name="page_slug_generate" value="1" id="page_slug_generate_true" /> <label class="none" for="page_slug_generate_true">{L_PAGES_PAGE_SLUG_GENERATE}</label><br />
...
$(document).ready(function(){
	$("#page_slug_generate_false").change(function(){
		...
		document.getElementById('page_slug_insert_div').id = '';
	});
});
		</script>
	</div>
