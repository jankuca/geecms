	<div id="content">
		<h2>{L_MODULE_USERS} &mdash; {L_USERS_USER_ADD}</h2>
...
<if(USER_GROUPS)>			<ul>
<foreach(USER_GROUPS)>			<li><input type="checkbox" name="user_groups[]" value="<var(GROUP_ID)>" id="user_groups_<var(GROUP_ID)>" /> <label for="user_groups_<var(GROUP_ID)>" class="none"><var(GROUP_HEADER)></label></li>
</foreach(USER_GROUPS)>		</ul>
<else(USER_GROUPS)>		<p>{L_USERS_NO_GROUPS}</p>
</if(USER_GROUPS)>
			<input type="submit" name="action" value="{L_USERS_USER_ADD}" id="user_submit" />
		</div>
		</form>
...
$(document).ready(function(){
	$("#user_password_input").click(function(){
